...
sub new {
    ...
    if (!$pluginObject) {
        logWarning("pluginObject parameter is required for ECPDF::ComponentManager initialization, consider to add it.");
    }
    my $self = {
        components_local => {},
    };
...
sub loadComponentLocal {
    ...
    if ($component->isEFComponent()) {
        if (!$pluginObject) {
            bailOut("Plugin Object is required if ECPDF::Component::EF::* or its subclass is being loaded");
        }
